...
public class Menu {
	...
	public static void suppressionMoniteur(String mail){
		...
		try{			
			...
			//On r�cup�re toutes les s�ances encadr�es par le moniteur
			pS = jdbc.getConnection().prepareStatement("SELECT * FROM Encadre WHERE mail=?");
			pS.setString(1, mail);
			rS = pS.executeQuery();
			//Pour chacune de ces s�ances
			while(rS.next()){
				String uidGroupe;	
				String uidSeance;
				//On r�cup�re la s�ance dans la table Seance
				...
				while(rS2.next()){
					int nbStagiaire = 0;
					//On r�cupere tous les stagiaires qui participatient a cette seance
					...
					//On r�cupere l'activit� li�e a cette s�ance pour r�cup�rer la nbmaxparmoniteur
					...
					int nbMoniteurs = 0;
					//On r�cup�re l'ensemble des moniteurs encadrant la s�ance
